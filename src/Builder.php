...
class Builder
{
    ...
    public function __construct(array $st_classes, $s_output_dir, $title = 'php-apidoc', $s_output_file = 'index.html', $template_path = null , $detail_domain = '')
    {
        ...
        $this->detail_domain = $detail_domain;
        ...
        foreach ($st_annotations as $class => $methods) {
            foreach ($methods as $name => $docs) {
                ...
                $tr = array(
                    ...
                    '{{ routeCode }}'               => sha1($docs['ApiRoute'][0]['name']),
                    '{{ description }}'             => $docs['ApiDescription'][0]['description'],
                    '{{ detailUrl }}'               => $this->detail_domain . "?m=" . sha1($docs['ApiRoute'][0]['name']),
...
//          array_unshift($value, '<h2>' . $key . '</h2>');
    ...
    public function getRouteDetail($route)
    {
    }
}
